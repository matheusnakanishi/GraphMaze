...
int dfs(Graph G, int start, int target, int *visited, int intermediate) {
    if (start == target)
        return 0;
    ...
    while (adjacent) {
        ...
        if (!visited[neighbor]) {
            int distance;
            if (neighbor == target && intermediate)
                distance = 0; // Se estamos no vértice intermediário, a distância é 0
            else
                distance = 1 + dfs(G, neighbor, target, visited, intermediate);
...
int shortest_path(Graph G, int start, int target) {
    ...
    int shortest = dfs(G, start, target, visited, 0);
...
void search(Graph G, int start, int target, int end) {
    ...
    printf("\nMinimum vertex to pick the cheese and leave the maze: %d\n", path1 + path2 - 1); // A subtração de 1 é para remover a contagem do vértice intermediário
}
